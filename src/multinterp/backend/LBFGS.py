...
class LBFGS(Optimizer):
    ...
    def __init__(
        ...
        if line_search not in ["Armijo", "Wolfe", "None"]:
            msg = f"Invalid line search: {line_search}"
            raise ValueError(msg)
    ...
    def _add_update(self, step_size, update) -> None:
        offset = 0
        for p in self._params:
            numel = p.numel()
    ...
    def _load_params(self, current_params) -> None:
        for i, param in enumerate(self._params):
            param.data[:] = current_params[i]

    def line_search(self, line_search) -> None:
    ...
    def curvature_update(self, flat_grad, eps=1e-2, damping=False) -> None:
        ...
        # load parameters
        if eps <= 0:
            msg = "Invalid eps; must be positive."
            raise (ValueError(msg))
    ...
    def _step(self, p_k, g_Ok, g_Sk=None, options=None):
        ...
        if line_search == "Armijo":
            ...
            if options:
                ...
                if "eta" not in options:
                    eta = 2
                elif options["eta"] <= 0:
                    msg = "Invalid eta; must be positive."
                    raise (ValueError(msg))
                else:
                    eta = options["eta"]
                ...
                if "c1" not in options:
                    ...
                    msg = "Invalid c1; must be strictly between 0 and 1."
                    raise (ValueError(msg))
                else:
                    c1 = options["c1"]

                if "max_ls" not in options:
                    max_ls = 10
                elif options["max_ls"] <= 0:
                    msg = "Invalid max_ls; must be positive."
                    raise (ValueError(msg))
                else:
                    max_ls = options["max_ls"]

                interpolate = options.get("interpolate", True)

                inplace = options.get("inplace", True)
        ...
        if line_search == "Wolfe":
            ...
            if options:
                ...
                if "eta" not in options:
                    ...
                    msg = "Invalid eta; must be greater than 1."
                    raise (ValueError(msg))
                else:
                    eta = options["eta"]
                ...
                if "c2" not in options:
                    c2 = 0.9
                elif options["c2"] >= 1 or options["c2"] <= 0:
                    msg = "Invalid c2; must be strictly between 0 and 1."
                    raise (ValueError(msg))
                elif options["c2"] <= c1:
                    msg = "Invalid c2; must be strictly larger than c1."
                    raise (ValueError(msg))
                else:
                    c2 = options["c2"]
            ...
            while True:
                ...
                if not interpolate or not is_legal(F_b):
                    t = eta * t if beta == float("Inf") else (alpha + beta) / 2.0
